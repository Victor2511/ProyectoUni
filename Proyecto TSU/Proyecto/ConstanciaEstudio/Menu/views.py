...
from biometria.utils import procesar_foto_carnet
import base64
...
def register_student(request):
    if request.method == 'POST':
        ...
        # Inicializar el formset aquí también para POST
        document_formset = DocumentosFormSet(request.POST, request.FILES)
        ...
        if form.is_valid():
            ...
            if document_formset.is_valid():
                ...
                # PROCESAR FOTO DE CÁMARA SI FUE ENVIADA
                foto_carnet_dataurl = request.POST.get('foto_carnet')
                if foto_carnet_dataurl:
                    try:
                        # Separar el encabezado 'data:image/jpeg;base64,' del contenido base64
                        format, imgstr = foto_carnet_dataurl.split(';base64,') 
                        image_data = base64.b64decode(imgstr)

                        # Convertir el base64 en un archivo en memoria
                        image_file = InMemoryUploadedFile(
                            BytesIO(image_data),         # archivo en memoria
                            field_name='foto_carnet',
                            name='foto_carnet.jpg',
                            content_type='image/jpeg',
                            size=sys.getsizeof(image_data),
                            charset=None
                        )

                        # Procesar la imagen
                        carnet_img = procesar_foto_carnet(image_file)

                        # Crear el documento con la imagen procesada
                        Documentos.objects.create(
                            estudiante=student,
                            tipo_documento='Foto Carnet',
                            archivo=carnet_img
                        )
                    except Exception as e:
                        messages.warning(request, f"Foto capturada no pudo ser procesada correctamente: {e}")
            ...
            # Asegurarnos de que document_formset esté definido incluso cuando form no es válido
